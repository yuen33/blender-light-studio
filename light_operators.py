...
from . import window_operations
import os

class Blender_Light_Studio_Properties(bpy.types.PropertyGroup):
    initialized = BoolProperty(default = False)
bpy.utils.register_class(Blender_Light_Studio_Properties)
bpy.types.Scene.BLStudio = PointerProperty(type = Blender_Light_Studio_Properties)
...
class AddBSLight(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        '''
        with bpy.data.libraries.load(dir+"/BLS_V1_02_simple.blend", link=False) as (src, dst):
            dst.groups = ["BLS_Light"]
        '''
...
class DeleteBSLight(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        '''
        # Because of nasty 2.75a bug. All appended objects have 2 users (WTF WHY?!) which makes removing from bpy.data.objects impossible
        bpy.ops.object.select_all(action='DESELECT')
        for ob in obsToRemove:
...
class PrepareBSLV3D(bpy.types.Operator):
    ...
    def execute(self, context):
        window_operations.splitV3DtoBLS(context)
